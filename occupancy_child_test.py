...
if __name__ == '__main__':
	#cap = cv2.VideoCapture("/home/shiraz/Desktop/Occupancy/Posestron/weights/demo.mp4")
	...
	while (cap.isOpened):
		...
		for box in bbox:
			...
			cv2.circle(frame, center=tuple(right_shoulder), radius=4, color=(0, 0, 225), thickness=6)
			cv2.circle(frame, center=tuple(left_shoulder), radius=4, color=(225, 0, 225), thickness=6)
			cv2.circle(frame, center=tuple(left_Belt), radius=4, color=(225, 0, 225), thickness=6)
			...
			print(f"distance 1 {distance_1}, distance_2 {distance_2}")
			...
			print(f"back person: {distance}")
			on_front = ["Driver", "Front passenger"]
			...
			if owner in on_front:
				...
				if distance > 100:         # 100 adult child flipping prefix 80
					res2 = "Adult " + owner 
				else:
					res2 = "Child " + owner 
	###################################################################
			# if owner in on_front:
			# 	if distance_1 < 180 and distance_2 < 90:
			# 		res = "Child " + owner 
			# 	else:
			# 		res = "Adult " + owner 
			# else:
			# 	if distance_1 < 85 and distance_2 < 60:
			# 		res = "Child " + owner 
			# 	else: 
			# 		res = "Adult " + owner 
	##################################################################
